...
const KNOWN_MESSAGES: Array<[RegExp, string]> = [
  [/only accepted prescriptions can be turned into an order/i, 'لا يمكن إنشاء طلب إلا من روشتة مقبولة'],
  ...
  [/status.*!=.*pending|already.*reviewed|cannot review/i, 'لا يمكن مراجعة روشتة تمت مراجعتها من قبل'],
  ...
  [/not.*(available|in stock)|isavailable.*false/i, 'أحد الأصناف المختارة غير متاح حاليًا'],
  [/not found in your pharmacy|not in.*pharmacy.*stock/i, 'أحد الأصناف المختارة غير موجود في مخزون صيدليتك'],
  [/invalid.*transition|not allowed.*status/i, 'لا يمكن تنفيذ هذا الإجراء في حالة الطلب الحالية'],
];
...
export function apiErrorMessageAr(err: unknown, fallback: string): string {
  const raw: string | undefined = (err as any)?.error?.message ?? (err as any)?.message;
